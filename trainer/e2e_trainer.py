...
class E2ETrainer(TFTrainer):
    ...
    def train(self, train_data_loader, eval_data_loader, metrics=('mask_rmse_np', 'Recall', 'MAP')):
        ...
        for epoch in range(1, self.max_epoch_num + 1):
            ...
            eval_loss, _, _ = self.evaluate(eval_data_loader, adj_mx, metrics)
    ...
    def model_loss_func(self, y_pred, y_true, encode, target_time, cur_adj_mx=None):
        ...
        if self.model.training:
            ...
            pred_loss = pred_loss + sim_loss + self.lam * contrast_loss(encode, self.risk_sim, target_time, self.grid_node_map, self.data_type)
